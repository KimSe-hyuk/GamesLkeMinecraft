...
// [핵심 추가 1] "이 청크는 다시 그려야 해!" 라는 표시(스티커)입니다.
#[derive(Component)]
pub struct NeedsRemesh;

// === 1. 로딩 시스템 (기존과 동일) ===
pub fn load_assets(mut commands: Commands, asset_server: Res<AssetServer>) {
    println!("=== [System] 텍스처 폴더 스캔 시작... ===");
...
pub fn check_assets_ready(
    ...
    if let Some(folder) = loaded_folders.get(&game_assets.folder_handle) {
        ...
        builder.max_size(UVec2::new(4096, 4096)); // 넉넉하게
        ...
        if let Some(atlas_layout) = atlases.get(&game_assets.atlas_layout) {
            ...
            for x in -RENDER_DISTANCE..=RENDER_DISTANCE {
                for z in -RENDER_DISTANCE..=RENDER_DISTANCE {
                    ...
                    if chunk_manager.loaded_chunks.contains(&chunk_coord) { continue; }
                    ...
                    for bx in 0..CHUNK_SIZE {
                        for bz in 0..CHUNK_SIZE {
                            ...
                            let height = ((wx as f32 * 0.05).sin() * 10.0 + (wz as f32 * 0.08).cos() * 8.0 + 15.0) as usize;
                            ...
                            for y in 0..height {
                                ...
                                if y == height - 1 { block_type = BLOCK_GRASS; }
                                    ...
                                    if y < 10 && chance < 0.05 { block_type = BLOCK_DIAMOND; }
                                    else if y < 20 && chance < 0.08 { block_type = BLOCK_GOLD; }
...
// === [핵심 추가 2] 더티 청크 리빌딩 시스템 ===
// "NeedsRemesh" 스티커가 붙은 청크만 찾아서 메쉬를 갈아끼웁니다.
pub fn rebuild_chunks(
    ...
    for (entity, coord, mut mesh_handle) in chunk_query.iter_mut() {
        // 1. VoxelWorld에서 현재 청크의 최신 데이터를 가져옴
        let mut chunk_data = vec![BLOCK_AIR; CHUNK_SIZE * CHUNK_SIZE * CHUNK_HEIGHT];
        ...
        // 3. 메쉬 갈아끼우기 (기존 핸들에 새 메쉬 할당)
        *mesh_handle = meshes.add(new_mesh);

        // 4. 스티커 제거 (이제 깨끗해졌으니까!)
        commands.entity(entity).remove::<NeedsRemesh>();
        
        println!("♻️ 청크 리빌드 완료: ({}, {})", coord.x, coord.z);
    }
}

// === 3. 메쉬 생성 (수정된 add_face 포함) ===
